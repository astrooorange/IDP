//list for all four objects + home
int object [] = {object1[],object2[],object3[],object4[],home[]};
 
//the car begins at node 1 facing north, and ends up at node 7 facing east
int object1 [] = {0,6,1,2,0,2};

//begins at node 8 facing south
int object2[] = {6,2,2,2};

//the car begins at node 8 facing south, ends up at node7 facing east
int object3 [] = {1,6,1,1,0,10};

//the car begins at node 8 facing south, ends up at node 7 facing east
int object4 [] = {1,2,2,6,0,2,0,2};

//the car begins at node 8 facing south 
int home []= {1,2,2,1,9};

//the car begins at node 7 facing east, and ends up at node 8 facing south; next one: object2 detection (node8)
int node7_magnetic[] = {2,2,4,1};
int node7_nonmagnetic[] = {0,2,4,2,1,0};

